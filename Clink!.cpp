...
void EraseObject() {
	// Ball, BallDeco[2], Crystal, Background, MakeCrashedCrystal, CrashedCrystal, Obstacle, CrashedObstacle, LosedBall;
...
bool FindIntersection(glm::vec2 p1, glm::vec2 p2, glm::vec2 p3, glm::vec2 p4, glm::vec2* intersection) {
	...
	a = (p1.x * p2.y - p1.y * p2.x) * (p3.x - p4.x);
	b = (p1.x - p2.x) * (p3.x * p4.y - p3.y * p4.x);
	c = (p1.x - p2.x) * (p3.y - p4.y);
	d = (p1.y - p2.y) * (p3.x - p4.x);
	x = (a - b) / (c - d);

	a = (p1.x * p2.y - p1.y * p2.x) * (p3.y - p4.y);
	b = (p1.y - p2.y) * (p3.x * p4.y - p3.y * p4.x);
	y = (a - b) / (c - d);
...
glm::vec3 CalFragmentVelocity(GLObj& ball, GLObj& fragment, const char mode) {
	...
	if (mode == 'c') {
		...
		glm::vec3 temp = fragment.midpos + fragment.pos;
		temp.y /= winSizex / winSizey;
		if (glm::distance(temp, ball.pos) > 0.175f)
			velocity = glm::vec3(0.f);
	}
...
std::vector <glm::vec3> ClampCrashVertex(std::vector <glm::vec3>& vertex, GLObj& ball, GLObj& obstacle) {
			...
			for (int dir = 0; dir < 4; dir++) {
				...
				if (dir == 0) {
					if (FindIntersection(
						...
						//cout << "선분\t(" << vertex[index].x << ", " << vertex[index].y << ")\t(" << vertex[i].x << ", " << vertex[i].y << ")" << endl;
...
void CrashObstacle(GLObj ball, GLObj& obstacle) {
	...
	for (int j = 0; j < angle.size(); j++) {
		...
		// 진짜 좌표 넣기
		vertex = ClampCrashVertex(vertex_temp, ball, obstacle);
		size = vertex.size();
...
GLvoid drawScene()
{
	...
	if (GameState != custom_s) {
		...
		glUniform1f(BgboolLocation, false);
		glUniform1f(UiboolLocation, false);
		Clink.Update();
	...
	if (GameState == end_s) {
		GameOver.Update();
		GameOver.draw_prepare(PosLocation, "Pos");
		GameOver.draw_prepare(WorldTransLocation, "World");
		GameOver.draw_prepare(NormalLocation, "Normal");
		GameOver.draw_prepare(UvLocation, "UV");
		GameOver.draw_prepare(false, "Texture");
		GameOver.draw_prepare(TexorColorLocation, "Texture_bool");
		glUniform1f(DistanceLocation, distance(Light.pos, GameOver.pos));
		GameOver.draw("solid");
	}
	...
	if (GameState == play_s) {
		...
		if (GameMode == clasic_m) {
			...
			for (int i = 0; i < number.size(); i++) {
				if (number[i] == '.') {
					print_dot = true;
					Number.back().pos = glm::vec3(-1.f + 0.286f * (0.2f * (float)(i - 1)) + (0.005f * (float)i) + 0.286f * 0.1f + 0.05f + 0.122f * 0.35f, 1.f - 0.175f, 0.f);
					Number.back().Update();
					Number.back().draw_prepare(PosLocation, "Pos");
					Number.back().draw_prepare(WorldTransLocation, "World");
					Number.back().draw_prepare(UvLocation, "UV");
					Number.back().draw_prepare(Number.back().now_img, "Texture");
					Number.back().draw_prepare(UiboolLocation, "UI_bool");
					Number.back().draw("solid");
				}
				else {
					Number[number[i] - '0'].pos = glm::vec3(-1.f + 0.286f * (0.2f * (float)(i - print_dot)) + (0.005f * (float)(i - print_dot)) + 0.286f * 0.1f + 0.05f + 0.122f * 0.3f * print_dot, 1.f - 0.175f, 0.f);
					Number[number[i] - '0'].Update();
					Number[number[i] - '0'].draw_prepare(PosLocation, "Pos");
					Number[number[i] - '0'].draw_prepare(WorldTransLocation, "World");
					Number[number[i] - '0'].draw_prepare(UvLocation, "UV");
			...
			for (int j = Number.size() - 3; j < Number.size() - 1; j++) {
				Number[j].pos = glm::vec3(-1.f + 0.286f * (0.2f * (float)(number.size() - print_dot + j - (Number.size() - 3))) + (0.005f * (float)(number.size() - print_dot + j - (Number.size() - 3))) + 0.286f * 0.1f + 0.05f + 0.122f * 0.3f * print_dot, 1.f - 0.175f, 0.f);
				Number[j].Update();
				Number[j].draw_prepare(PosLocation, "Pos");
				Number[j].draw_prepare(WorldTransLocation, "World");
				Number[j].draw_prepare(UvLocation, "UV");
				Number[j].draw_prepare(Number[j].now_img, "Texture");
				Number[j].draw_prepare(UiboolLocation, "UI_bool");
				Number[j].draw("solid");
			}
		}
	}
		...
		if (button == GLUT_LEFT_BUTTON) {
			...
			for (int i = 0; i < Ui[GameState].size(); ++i) {
				if (m.x >= Ui[GameState][i].leftbottom.x * Ui[GameState][i].scale.x + Ui[GameState][i].pos.x &&
					...
					UiClick(i);
					skip = true;
					if (GameState != Now_state || (GameState != option_s && GameState != stop_s) || i != 0)
						break;
				}
			}
...
void Init()
{
	...
	// GameOver
	{
		std::ifstream inputFile("./OBJ/Gameover.obj");

		if (inputFile.is_open())
			GameOver.objLoad(inputFile);
		else
			std::cerr << "Failed to obj file" << std::endl;

		GameOver.scale = glm::vec3{ 0.004, 0.004, 0.004 };
		GameOver.pos = glm::vec3{ -0.1f, 0.2f, 0.f };

		GameOver.imgLoad("./IMG/유리.png");
	}
	...
	// Ui
	{
		...
		Ui[end_s].emplace_back(SetImageSize(800, 250));
		Ui[end_s].back().imgLoad("./IMG/title_go.png");
		Ui[end_s].back().scale = glm::vec3(0.5f);
		Ui[end_s].back().pos = glm::vec3(0.f, -0.6f, 0.f);
	}
